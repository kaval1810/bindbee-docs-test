...
<Note>
  A `200` from this endpoint confirms Bindbee reached the vendor. Many APIs also return `200` alongside an error object in the body, in that system's own format - so inspect the payload, and handle status codes and error bodies per integration.
</Note>
