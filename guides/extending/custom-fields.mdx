...
<Warning>
Custom fields are **not returned by default.** A mapped field appears only when you pass `include_custom_fields=true`, which is the usual reason a mapping that is working looks like it failed - see [Add raw_data & Custom Fields](/guides/reading-writing/raw-data).
</Warning>
...
Custom fields extend a unified model with extra attributes, populated by a [JMESPath](https://jmespath.org/) expression pointed at the raw payload. This is the visual flow - for automation or version-controlled configuration, use the [API](/guides/extending/custom-fields) instead.

<Info>
  **Before you start**

  - You know the value exists upstream - see [Inspect raw data](/guides/reading-writing/raw-data).
...
<AccordionGroup>
  <Accordion title="The field returns INVALID_JSON_PATH">
    ...
    Usually an array needing an index or filter, or a quoting error inside a filter. Preview the expression before saving to catch it - see [Test an expression](/guides/extending/custom-fields).
  </Accordion>
  ...
  <Accordion title="The field is empty for some employees and populated for others">
    ...
    If it's empty for employees who should have it, the value may sit at a different path for those records, or be gated by permission. See [Permission errors](/guides/troubleshooting/permission-errors).
  </Accordion>
  ...
  <Accordion title="You need the same fields in both environments">
    Configuration doesn't cross environments. Recreating it by hand is error-prone at any scale - define it through the API instead so the same definitions can be applied to both. See [Map with the API](/guides/extending/custom-fields).
  </Accordion>
</AccordionGroup>
...
<Steps>
  ...
  <Step title="Inspect the raw payload">
    ...
    See [Inspect raw data](/guides/reading-writing/raw-data) for reading the response and building an expression.

    **Result:** A candidate JMESPath.
  </Step>

  <Step title="Validate before persisting">
    Dry-run the expression - see [Test an expression](/guides/extending/custom-fields). Skipping this is how `INVALID_JSON_PATH` reaches production responses.
...
<Info>
  **Before you start**
  ...
  - You have a candidate expression - see [Inspect raw data](/guides/reading-writing/raw-data).
</Info>
...
<Steps>
  ...
  <Step title="Test against a record that has the value">
    ...
    Preview against a connector where you know the value exists. In the dashboard, employee search lets you pick a specific person's record for the same purpose - see [Map from the dashboard](/guides/extending/custom-fields).

    **Result:** Confirmation the expression works where the data exists.
  </Step>

  <Step title="Save it">
    Attach the validated expression as a mapping - see [Map with the API](/guides/extending/custom-fields).
...
<AccordionGroup>
  ...
  <Accordion title="raw_data_source keeps returning integration_sample">
    The connector hasn't synced. Trigger one - see [Force a resync](/guides/reading-writing/syncing) - then preview again. On a Development connector nothing syncs until you trigger it.
  </Accordion>
...
<Info>
  **Before you start**
  ...
  - You have a custom field created - see [Map with the API](/guides/extending/custom-fields).
</Info>
...
<Steps>
  ...
  <Step title="Verify against more than one connector">
    ...
    Preview the expression against several connectors before assuming an org mapping holds - see [Test an expression](/guides/extending/custom-fields).
...
- [Raw Data](/guides/reading-writing/raw-data) - finding the value to map
- [Passthrough](/guides/extending/passthrough) - when a custom field is not enough
- [Custom Fields API](/api-reference/custom-fields/create-custom-field) - the endpoints
