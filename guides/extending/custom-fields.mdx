...
Whichever route you use, what you send on the mapping decides its type. Send `integration_slug` and Bindbee maps at the integration level; send `connector_token` and it maps at the connector level.

## Retrieve custom fields

Once a field has a mapping that resolves on a connector, add `include_custom_fields=true` to any unified request, such as [Get Employees](/hris/employee/get-employees).
...
4. **Automation**
   ...
   - Use the [API Configuration](/guides/extending/custom-fields/api-workflow) flow to keep custom field configuration in version control or replicate it across environments (dev → staging → prod).
