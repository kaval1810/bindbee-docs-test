...
<Steps>
  ...
  <Step title="Create the connector">
    ...
    **Result:** The customer authorises, and the connector moves to `COMPLETE`. See [Connection methods](/get-started/connection-methods).
  </Step>

  <Step title="(Optional) Subscribe to connector_sync_started">
  ...
  <Step title="(Optional) Poll the Connectors API">
  ...
  <Step title="(Optional) Fetch attached custom fields">
  ...
  <Step title="Subscribe to connector_synced">
    **Recommended.** This is the signal that a sync has finished and the data is ready to read. Verify the signature on every request and reply within 10 seconds.
...
<AccordionGroup>
  <Accordion title="Nothing arrives after the customer authorises">
...
- [Webhooks](/guides/reading-writing/webhooks) - the event catalogue behind steps 3 and 6
