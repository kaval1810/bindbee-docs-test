...
<Steps>
  ...
  <Step title="Expand the relations the employee actually carries">
    ...
    `expand` only works on ID fields already on the employee record - `manager`, `company`, `groups`, `work_locations`, `pay_group`. List them with commas and no spaces, and narrow each with `[field,field]` to keep the response small. See [Expand](/guides/reading-writing/reading-data/expand).
...
- [Reading data](/guides/reading-writing/reading-data) - filters, `expand` and `modified_after`
