...
<AccordionGroup>
  ...
  <Accordion title="Events arrive intermittently">
    Almost always slow acknowledgement. Every non-`2xx` and every network-layer failure counts against a short retry budget - see [Signatures & delivery](/guides/reading-writing/webhooks#retry-mechanism) for the exact window and timeout.
