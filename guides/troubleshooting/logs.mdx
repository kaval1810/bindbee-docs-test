...
Every request between Bindbee and a source system is recorded here: which API was called, when, and what it returned.

Come to this page once [sync status](/guides/troubleshooting/sync-status) and the [error's source](/guides/troubleshooting/errors) have left the question open.
...
**Webhook delivery logs are a separate surface.** They record Bindbee's attempts to reach *your* endpoint, so they answer why your handler stayed quiet. They do have an API: [List Webhook Logs](/api-reference/webhooks/list-webhook-logs).
