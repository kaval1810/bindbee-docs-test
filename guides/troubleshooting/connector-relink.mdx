...
description: "Re-authorise a connector whose credentials stopped working."
icon: "unlink"
---
...
<Steps>
  <Step title="Open the connector">
    ...
    **Result:** You can see its current status and sync history.
    ...
    <Frame>
      <img src="/images/platform/connectors-list.png" alt="The Connectors list with counters reading Active Connectors 52, Incomplete Connectors 130 and Relink Needed 21, above a table of connections" />
    </Frame>
  </Step>

  <Step title="Generate a relink">
    A connector needing re-authorisation carries a **Relink Needed** banner with the date it entered that state, and **Force Sync** is disabled. Click **Relink Connector** to produce an updated Magic Link for the same connector.

    **Result:** A link the customer can open to re-enter credentials against the existing connector.

    <Frame>
      <img src="/images/platform/connector-relink-needed.png" alt="A connector page showing a Relink Needed banner with the date since it entered that state and a Relink Connector button, a greyed-out Force Sync control, and a sync history whose most recent run is marked Failed" />
    </Frame>
  </Step>
  ...
  <Step title="Resync and verify">
    ...
    **Result:** The connector is syncing again and the data is current.
    ...
    <Note>
      ...
      connector rather than the connector itself, so everything your system has stored stays valid.
    </Note>
  </Step>
</Steps>
...
Both actions look like they mean "fix it". Only one does.
...
**Relinking** keeps the connector's identity, history, records and `connector_token`. Nothing you stored needs updating, and syncing resumes where it left off. This is the right response to essentially every broken connector.
...
The expensive part is the days before anyone notices. Subscribe to the **Connector Sync Error** webhook event (`connector_sync_error`) and a stalled connector reaches you the day it breaks - see [Monitoring this programmatically](/guides/troubleshooting/sync-status#monitoring-this-programmatically).
