...
A customer says they have 4,200 employees; you're seeing 4,417. The two numbers are counting different populations.

Establish what each one counts before investigating a fault.
...
<Steps>
  ...
  <Step title="Break the count down by status">
    ...
    This one breakdown resolves most gaps. Customers quote active permanent employees; an unfiltered API count includes everyone.
  ...
  <Step title="Only then treat it as a data problem">
    A gap that survives all of the above becomes a missing-records investigation - see [Missing records](/guides/troubleshooting/missing-records).

    A permission gap can hide a whole population: a legal entity the credentials exclude disappears silently, with every call succeeding.
...
<AccordionGroup>
  ...
  <Accordion title="Counts match for employees but not employments">
    Expected. Employments exceed employees wherever anyone has been rehired, transferred, or holds concurrent roles.
  </Accordion>
...
- [Reading benefits](/guides/data-models/benefits) - why plans and enrolments count differently
