...
Writing does not go through the copy. **A write is submitted to the source system and reports what came back**, which makes it asymmetric with reading in four ways.

```mermaid
flowchart LR
    YOU["Your application"] -->|"GET meta"| BB["Bindbee"]
    BB -.->|"the schema this connector needs"| YOU
    YOU -->|POST| BB
    BB -->|write| SRC["Source system"]
    SRC -.->|"its own answer"| BB
    BB -.->|"in Bindbee's envelope"| YOU
