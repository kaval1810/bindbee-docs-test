...
**That you received everything.** Delivery is best-effort and the retry window is 60 seconds. A deploy that takes your endpoint down for two minutes loses those events permanently, with no error in your logs or Bindbee's. Do not try to recover the events — take the earliest plausible start and latest plausible end of the outage and read current state with `modified_after` for each affected connector and model, as in [Querying Data](/guides/reading-writing/querying-data). If a record changed three times during the outage, current state is what you wanted anyway.

**Which field changed.** An event says a record changed, not which attribute or what it was before.

**Whether a record was created or updated.** Data-change events say a record changed, not which — there is no flag in the payload, so compare against your own stored copy. The first sync is the case to design for: it loads the entire population, so everything classifies as created. Process it as a bulk import rather than triggering per-record workflows, or a newly connected customer gets a welcome email sent to their whole workforce.
...
{/* VERIFY: a created-versus-modified flag in the payload was discussed with engineering in 2026. If it shipped, make it the primary method and demote comparing against a stored copy. */}
