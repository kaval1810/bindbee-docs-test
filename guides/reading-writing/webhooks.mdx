...
When a webhook event is triggered, Bindbee sends a `POST` request to your configured destination URL. If the request fails, Bindbee automatically retries the delivery **up to 4 times within 60 seconds**.
...
{/* VERIFY: does a non-2xx from a reachable endpoint consume a retry, or only request and
    network-layer failures? The two statements on the live page disagree - its Retry trigger
    row says "and all non-2xx responses" while the section above the failure table says
    retries fire "only for request or network-layer failures". Kept the inclusive reading
    here because connections-to-relink.mdx already tells readers every non-2xx counts against
    the budget, but one of the three is wrong and it decides whether a handler that returns
    500 gets redelivered. */}
