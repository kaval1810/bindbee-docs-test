...
These pages use [Get Employees](/hris/employee/get-employees) throughout, but the same behavior applies across HRIS, Payroll, ATS and LMS endpoints. The exact parameter set differs per model, so check the reference page for the endpoint you are calling.
