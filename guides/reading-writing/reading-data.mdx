...
Every list endpoint returns data Bindbee has already synced from the connected system. **A read is served from that copy, not from a live call to the source system.**
...
These pages use [Get Employees](/hris/employee/get-employees) throughout, but the same behaviour applies across HRIS, Payroll, ATS and LMS endpoints. The exact parameter set differs per model, so check the reference page for the endpoint you are calling.
