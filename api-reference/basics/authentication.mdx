...
When issuing requests to the Bindbee API concerning your end users' data, access or manipulation, authorization is granted only if the users have undergone the [Bindbee's Magic Link](/how-to/connections/generate-a-magic-link) process and successfully stored their `connector_token`, as this token is a prerequisite for the successful execution of such requests.
