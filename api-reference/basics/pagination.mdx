...
The cheapest way to page less is to fetch less: filter server-side instead of discarding client-side, and on scheduled runs use `modified_after` to fetch only what changed. See [modified_after](/guides/reading-writing/reading-data/modified-after).
