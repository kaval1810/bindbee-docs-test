...
<AccordionGroup>
  ...
  <Accordion title="You need to backfill a large volume">
    Write sequentially with per-entry idempotency keys rather than in parallel, and watch the [rate limit](/guides/api-basics/rate-limits) of 200 requests per minute per connector token. Partial failure mid-backfill is easier to resume with deterministic keys.
  </Accordion>
</AccordionGroup>
