...
<Steps>
  ...
  <Step title="Detect loss of coverage from termination">
    ...
    `INACTIVE`, `RETIRED`, `DECEASED` and `INACTIVE_EXTERNAL` are all departures - see [Cascade a Termination Downstream](/get-started/use-cases/identify-a-termination) for handling them properly.
...
<AccordionGroup>
  ...
  <Accordion title="date_of_birth is null for every dependent">
    Almost always a permission, not absence - sensitive fields are commonly gated behind a separate grant. Without it, aging-out cannot be detected at all. See [Permission errors](/guides/troubleshooting/permission-errors).
  </Accordion>
...
- [Cascade a Termination Downstream](/get-started/use-cases/identify-a-termination)
