...
<Info>
  **Before you start**
  ...
  - You have confirmed the integration supports candidate writes - see [Check write support](/guides/reading-writing/meta-apis). Probe `GET /api/ats/v1/candidates/create/meta`.
...
<Steps>
  <Step title="Fetch the write schema">
    ...
    Build the body from this rather than hardcoding it - required fields differ per integration, and a required `integration_params` block means the write needs platform-specific values you must source first. See [Fetch a schema](/guides/reading-writing/meta-apis).
...
<AccordionGroup>
  <Accordion title="The write returned 501">
    The Meta schema isn't published for this integration. That is not the same as the write being unsupported - see [Meta APIs](/guides/reading-writing/meta-apis#which-operations-have-a-schema).
  </Accordion>
...
- [Check write support](/guides/reading-writing/meta-apis)
