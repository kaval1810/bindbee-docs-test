...
<Steps>
  ...
  <Step title="Page both collections fully">
    ...
    Follow the cursor until it stops coming back, and don't stop on a short page - see [Pagination](/guides/api-basics/pagination).
  ...
  <Step title="Subscribe to changes on both models">
    ...
    Re-read dependents and dependent benefits whenever the employee changes. `modified_after` is per model, so an employee changing does not mark their dependents as changed - see [Filter with modified_after](/guides/reading-writing/querying-data).
...
<AccordionGroup>
  ...
  <Accordion title="date_of_birth is null for every dependent">
    A sensitive-data permission rather than absence. See [Permission errors](/guides/troubleshooting/permission-errors).
  </Accordion>
