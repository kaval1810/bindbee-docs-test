...
<AccordionGroup>
  <Accordion title="Every employee arrived as a new hire">
    A first sync or a resync. Both re-sync the whole population, so every record looks changed - see [Filter with modified_after](/guides/reading-writing/querying-data) for why. This is what the baseline guard in step 2 exists to prevent.
  </Accordion>
