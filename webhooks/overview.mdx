...
description: "The five webhook events, the payload each one delivers, the retry policy, and how to verify a signature."
---

The five events Bindbee can send, what each payload contains, and how deliveries are retried and signed.

To create a webhook see [Create a Webhook](/how-to/reliability/create-a-webhook); for what each field on the Dashboard screen does see [Webhooks](/platform/overview).
...
if (!receivedSignature) {
  // Fail closed: never fall through to comparison without a signature
  throw new Error("Missing webhook signature; request may not be from Bindbee.");
}

// Create an HMAC digest using SHA-256 over the raw body
