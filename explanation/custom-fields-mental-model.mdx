...
<Warning>
  ...
  tool. You need [passthrough](/explanation/passthrough-when-and-why) — a live
...
| --- | --- | --- |
| **Organization** | `integration_slug` | Every connector of that integration in your org |
...
The intended pattern: define an org-level mapping that works for the typical customer on that integration, and add connector-level overrides for the customers who configured their HRIS unusually. That is common — HRIS deployments are heavily customized, and two BambooHR customers can store "emergency contact" in different custom-field slots.
...
| --- | --- |
| Value has a unified field that is `null` | [Diagnose the null](/explanation/field-semantics) — a mapping won't fix a permissions problem |
| You need a whole vendor endpoint | [Passthrough](/explanation/passthrough-when-and-why) |
| You want to *write* the value | Custom fields are read-only — see [Write Data Back](/get-started/tutorials/write-data-back) |
