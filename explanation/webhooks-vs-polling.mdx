...
<Warning>
  ...
  [Data Freshness](/explanation/how-syncing-works).
</Warning>
...
**Use both.** Webhooks for latency, a scheduled sweep for correctness. Both do the same thing: read with `modified_after=<watermark>`, then upsert. The webhook triggers that read immediately after a sync; the sweep runs the same read again on a timer (daily is usually sufficient), so nothing is lost if a webhook is missed.
...
- **Verify the signature** against the raw request body. See [Validate webhook signatures](/how-to/reliability/validate-webhook-signatures).
...
<CardGroup cols={2}>
  <Card title="Choose Webhook Events" icon="webhook" href="/how-to/reliability/choose-webhook-events">
    Which events to subscribe to, once you've decided to stop polling.
  </Card>
