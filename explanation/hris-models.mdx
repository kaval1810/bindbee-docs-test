...
<Warning>
Do not read the first employment in the array and treat it as current. Ordering is not guaranteed to mean anything. Employment carries `effective_date` and no end date of its own, so the current one is the one with the latest `effective_date`.

Whether the person still works there is a separate, employee-level question: `employment_status`, `termination_date` and `termination_reason` all sit on the employee, not on any employment.
</Warning>
