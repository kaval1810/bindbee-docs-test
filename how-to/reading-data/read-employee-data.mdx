...
The employee model is the entry point for most integrations. This page covers what's specific to it - for the general techniques, see [Paginate Through Results](/how-to/reading-data/paginate-through-results) and [Filter and Narrow a Collection](/how-to/reading-data/filter-and-narrow-a-collection).
...
<Steps>
  ...
  <Step title="Expand related models instead of fetching them separately">
    ...
    `work_locations` expands into the [Location](/hris/locations/get-locations) model - name, street, city, state, postal code and country - which is what you need for tax jurisdiction or eligibility by state.
  ...
  <Step title="Ask for raw data or custom fields explicitly">
    ...
    `include_raw_data` is a debugging tool. Leaving it on in production inflates every response substantially - see [Inspect the Raw Upstream Payload](/how-to/data-model/inspect-the-raw-upstream-payload) for the targeted alternative.
...
<AccordionGroup>
  <Accordion title="A field is null for every employee">
    ...
    See [Resolve a Source-System Permission Error](/how-to/troubleshooting/resolve-a-permission-error).
  </Accordion>

  <Accordion title="One person appears twice">
    Usually two employment records rather than two employees, or a genuine duplicate from an upstream identifier change. See [Trace an Unexpected or Duplicate Record](/how-to/troubleshooting/trace-an-unexpected-record).
  </Accordion>

  <Accordion title="Terminated employees are still returned">
    Expected - records persist after termination and carry a terminal status. Filter on `employment_status` for the population you want, and read the termination date from the employment record.
  </Accordion>

  <Accordion title="The population is larger than the customer's headcount">
    Contingent workers and contractors are usually the difference. They commonly carry `ACTIVE_EXTERNAL`. See [Reconcile Record Counts Against the Source](/how-to/troubleshooting/reconcile-record-counts).
  </Accordion>
