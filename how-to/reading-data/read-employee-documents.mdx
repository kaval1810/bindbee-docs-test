...
<AccordionGroup>
  ...
  <Accordion title="data_status has been PENDING for a long time">
    Extraction runs after the document syncs. Subscribe to `document.data.available` rather than polling the record - see [Choose Webhook Events](/webhooks/events). If it never resolves, download the file; the document is usable regardless of whether parsing succeeded.
  </Accordion>
