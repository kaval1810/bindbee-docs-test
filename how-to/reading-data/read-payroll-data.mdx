...
For hours worked rather than money paid, see [Reading time & attendance](/how-to/reading-data/read-time-and-attendance-data) - the two do not reconcile arithmetically, and [Payroll & time models](/explanation/payroll-and-time-models) explains why.
...
The calendar matters more than it looks. Deduction changes have to land before the customer's processing cut-off, and the calendar is how you know when the next run is - see [Employee payroll run](/how-to/writing-data-back/create-an-employee-payroll-run).
...
<Steps>
  ...
  <Step title="Read deductions from the per-employee model">
    ...
    Those codes are customer-specific and do not map automatically to benefit plans - the relationship is configuration you establish with the customer and store yourself. See [Reading benefits](/how-to/reading-data/read-benefits-data) for the plan side.
...
<AccordionGroup>
  <Accordion title="Payroll returns nothing though the customer runs payroll">
    Payroll is usually a separately licensed module with its own permission grant, and an empty result is the common failure mode rather than an error. See [Permission errors](/how-to/troubleshooting/resolve-a-permission-error).
  </Accordion>
  ...
  <Accordion title="Payroll doesn't reconcile with hours worked">
    Expected. Overtime is calculated by rules the timesheet doesn't encode, salaried employees are paid without timesheets at all, and corrections land in a later run than the period they fix. See [Reading time & attendance](/how-to/reading-data/read-time-and-attendance-data).
  </Accordion>
</AccordionGroup>
