...
<Info>
  **Before you start**
  ...
  - You know the value exists upstream - see [Inspect raw data](/how-to/data-model/inspect-the-raw-upstream-payload).
...
<AccordionGroup>
  <Accordion title="The field returns INVALID_JSON_PATH">
    ...
    Usually an array needing an index or filter, or a quoting error inside a filter. Preview the expression before saving to catch it - see [Test an expression](/how-to/data-model/test-a-jmespath).
  </Accordion>
  ...
  <Accordion title="You need the same fields in both environments">
    Configuration doesn't cross environments. Recreating it by hand is error-prone at any scale - define it through the API instead so the same definitions can be applied to both. See [Map with the API](/how-to/data-model/map-custom-fields-with-the-api).
  </Accordion>
</AccordionGroup>
