...
sidebarTitle: "Test an expression"
...
<Info>
  **Before you start**
  ...
  - You have a candidate expression - see [Inspect raw data](/how-to/data-model/inspect-the-raw-upstream-payload).
</Info>
...
<Steps>
  ...
  <Step title="Test against a record that has the value">
    ...
    Preview against a connector where you know the value exists. In the dashboard, employee search lets you pick a specific person's record for the same purpose - see [Map from the dashboard](/how-to/data-model/map-a-custom-field-from-the-dashboard).

    **Result:** Confirmation the expression works where the data exists.
  </Step>

  <Step title="Save it">
    Attach the validated expression as a mapping - see [Map with the API](/how-to/data-model/map-custom-fields-with-the-api).
