...
New here? [Map Custom Fields](/how-to/data-model/map-custom-fields) is the end-to-end walkthrough - this page is the full reference for the API path it links to.

The Custom Fields API exposes the same capabilities as the dashboard so you can define fields and mappings programmatically. For the concepts (what Custom Fields are, integration vs connector scope) see the [Custom Fields overview](/explanation/custom-fields). For the dashboard equivalent, see [Dashboard Configuration](/how-to/data-model/map-a-custom-field-from-the-dashboard).
...
- **Mapping** - the rule that tells Bindbee where to read the value from in the raw upstream payload. A mapping is either:
  - **Integration-scoped** (`integration_slug` set) - applies to every connector of that integration in your org.
  - **Connector-scoped** (`connector_token` set) - applies to a single connector instance and overrides the integration-level mapping.
...
<Steps>
  ...
  <Step title="Create a mapping">
    Attach the field to either an integration (integration-scoped) or a single connector
    (connector-scoped).
  </Step>
...
A mapping pairs the custom field with a JMESPath, scoped either to an integration (applies to every connector of that integration) or to a single connector (overrides the integration-level mapping for that connector only).

<Tabs>
  <Tab title="Integration-scoped">
  ...
  <Tab title="Connector-scoped">
    Applies to one specific connector and overrides any integration-level mapping
...
The configuration endpoint returns every custom field for a given `(connector, category, model)` together with the **effective** mapping in use. Connector-scoped mappings override integration-scoped ones, and unmapped fields are included with `json_path: null` so you can see what's still left to configure.
...
- `"integration"` - no connector override; the integration-scoped mapping is being inherited.
