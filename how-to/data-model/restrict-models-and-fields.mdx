...
Scoping controls what a connector syncs. Narrowing it reduces sync time, reduces load on the source system, and - the reason most customers ask - keeps sensitive data you have no use for out of your systems entirely.

<Warning>
  Confirm current scoping behaviour before relying on this page.

  {/* VERIFY: (1) is scoping configured per connector or shared across the org? (2) is it shared across environments? (3) does Platform Tour or this page own the click path? Answers change the steps below. */}
</Warning>
...
<Steps>
  ...
  <Step title="Apply the scope">
    Configure the connector's scope from its settings in the dashboard.

    **Result:** The connector is limited to the selected models and fields.
  </Step>
