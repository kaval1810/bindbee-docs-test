...
When credentials rotate, expire, or lose access, the connector stops syncing. Relink it rather than creating a new one - relinking keeps the connector ID and its record history, so the IDs your system has stored stay valid.
...
<Steps>
  ...
  <Step title="Check whether the connector token changed">
    Confirm the `connector_token` on the Overview tab still matches what your system has stored, and update it if not.

    {/* VERIFY: confirm whether relinking rotates connector_token. If it never does, cut this step. */}

    **Result:** Your stored credentials match the connector.
  </Step>
</Steps>

## Preventing the silent version of this

The failure mode that costs the most is not the relink - it's the days before anyone notices. A connector that needs re-authentication stops updating, but reads keep returning the last successful sync, so downstream systems process stale data as if it were current.
