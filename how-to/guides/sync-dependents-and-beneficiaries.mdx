...
<Steps>
  ...
  <Step title="Page both collections fully">
    ...
    Follow the cursor until it stops coming back, and don't stop on a short page - see [Pagination](/how-to/reading-data/paginate-through-results).
  ...
  <Step title="Map attributes the model doesn't carry">
    ...
    Find the path with the raw-data endpoint, then map it - see [Inspect raw data](/how-to/data-model/inspect-the-raw-upstream-payload). Read the results with `include_custom_fields=true`.
...
<AccordionGroup>
  ...
  <Accordion title="Dependents exist but no dependent benefits">
    Either they genuinely aren't enrolled, or the integration doesn't expose dependent coverage. Check `raw_data` on someone you know is covered - see [Inspect raw data](/how-to/data-model/inspect-the-raw-upstream-payload).
  </Accordion>
