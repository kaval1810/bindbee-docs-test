...
<Steps>
  ...
  <Step title="Page both collections fully">
    ...
    Follow the cursor until it stops coming back, and don't stop on a short page - see [Paginate Through Results](/how-to/reading-data/paginate-through-results).
  ...
  <Step title="Map attributes the model doesn't carry">
    ...
    Find the path with the raw-data endpoint, then map it - see [Inspect the Raw Upstream Payload](/how-to/data-model/inspect-the-raw-upstream-payload). Read the results with `include_custom_fields=true`.
  ...
  <Step title="Subscribe to changes on both models">
    ...
    Re-read dependents and dependent benefits whenever the employee changes. `modified_after` is per model, so an employee changing does not mark their dependents as changed - see [Filter with modified_after](/how-to/reading-data/filter-with-modified-after).
...
<AccordionGroup>
  ...
  <Accordion title="Dependents exist but no dependent benefits">
    Either they genuinely aren't enrolled, or the integration doesn't expose dependent coverage. Check `raw_data` on someone you know is covered - see [Inspect the Raw Upstream Payload](/how-to/data-model/inspect-the-raw-upstream-payload).
  </Accordion>

  <Accordion title="date_of_birth is null for every dependent">
    A sensitive-data permission rather than absence. See [Resolve a Source-System Permission Error](/how-to/troubleshooting/resolve-a-permission-error).
  </Accordion>
