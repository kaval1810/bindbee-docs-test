...
<Steps>
  <Step title="Subscribe to both change events">
    `employee_data_changed` covers status and employment; `connector_data_modified` covers the other models, including dependents - see [Choose Webhook Events](/how-to/reliability/choose-webhook-events).
  ...
  <Step title="Detect loss of coverage from termination">
    ...
    `INACTIVE`, `RETIRED`, `DECEASED` and `INACTIVE_EXTERNAL` are all departures - see [Cascade a Termination Downstream](/how-to/guides/cascade-a-termination-downstream) for handling them properly.
  ...
  <Step title="Find who else loses coverage">
    ...
    A dependent record means the employee recorded that person; a dependent benefit means they were actually enrolled - see [Read Benefits Data](/how-to/reading-data/read-benefits-data).
...
<AccordionGroup>
  ...
  <Accordion title="date_of_birth is null for every dependent">
    Almost always a permission, not absence - sensitive fields are commonly gated behind a separate grant. Without it, aging-out cannot be detected at all. See [Resolve a Source-System Permission Error](/how-to/troubleshooting/resolve-a-permission-error).
  </Accordion>
...
- [Cascade a Termination Downstream](/how-to/guides/cascade-a-termination-downstream)
