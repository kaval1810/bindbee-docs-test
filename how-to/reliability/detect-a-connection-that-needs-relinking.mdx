...
Three gaps are now covered: a failed sync, an expired credential, and a sync that never started. The one still open is reading data mid-sync - for that, gate your job on `connector_synced` rather than on a clock. See [Trigger a Job When a Sync Completes](/how-to/sync-control/trigger-a-job-on-sync-complete).
...
- [Webhook Events & Payloads](/webhooks/overview) - events, payloads, and retry behaviour
