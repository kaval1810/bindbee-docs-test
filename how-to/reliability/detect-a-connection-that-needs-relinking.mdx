...
sidebarTitle: "Connections needing relink"
...
<Steps>
  ...
  <Step title="Read the connector status from the payload">
    ...
    <Note>
      ...
      [How connections work](/explanation/connector-lifecycle).
    </Note>
  ...
  <Step title="Route the connection to your own re-auth flow">
    ...
    To generate a fresh connection link, see [Update credentials](/how-to/connections/update-credentials).
...
Three gaps are now covered: a failed sync, an expired credential, and a sync that never started. The one still open is reading data mid-sync - for that, gate your job on `connector_synced` rather than on a clock. See [Trigger a job on completion](/how-to/sync-control/trigger-a-job-on-sync-complete).
...
- [Events & payloads](/webhooks/overview) - events, payloads, and retry behaviour
