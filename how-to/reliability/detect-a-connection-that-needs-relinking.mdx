...
<Steps>
  <Step title="Subscribe to the sync error event">
    Create a webhook for the environment you're operating in and select **Sync error**. See [Create a Webhook](/webhooks/overview).
  ...
  <Step title="Read the connector status from the payload">
    ...
    <Note>
      ...
      [How connections work](/explanation/magic-link).
    </Note>
  ...
  <Step title="Route the connection to your own re-auth flow">
    ...
    To generate a fresh connection link, see [Update credentials](/how-to/reliability/detect-a-connection-that-needs-relinking).
...
Three gaps are now covered: a failed sync, an expired credential, and a sync that never started. The one still open is reading data mid-sync - for that, gate your job on `connector_synced` rather than on a clock. See [Trigger a job on completion](/explanation/how-syncing-works).
...
<AccordionGroup>
  ...
  <Accordion title="Events arrive intermittently">
    Almost always slow acknowledgement. Every non-`2xx` and every network-layer failure counts against a short retry budget - see [Signatures & delivery](/webhooks/overview#retry-mechanism) for the exact window and timeout.
  ...
  <Accordion title="The error message is generic">
    `error.message` can be a summary, with `detail` returned as an empty object when nothing more specific is available. Use the connector's sync history for the per-model breakdown - see [Partial syncs](/how-to/troubleshooting/diagnose-a-partial-sync).
  </Accordion>
</AccordionGroup>
...
<Info>
  **Before you start**
  ...
  - You've confirmed the failure is credential-related, not a permission gap - see [Permission errors](/how-to/troubleshooting/resolve-a-permission-error).
...
<Steps>
  <Step title="Open the connector">
    Find it under **Connectors** and open its page - see [Connectors](/platform/overview).
  ...
  <Step title="Resync and verify">
    Once they've reconnected, trigger a sync rather than waiting for the next scheduled run - see [Force a resync](/explanation/how-syncing-works).
...
The failure mode that costs the most is not the relink - it's the days before anyone notices. A connector in `RELINK_NEEDED` stops updating, but reads keep returning the last successful sync, so downstream systems process stale data as if it were current. Nothing on the data endpoints errors - see [How connections work](/explanation/magic-link) for why the data is kept rather than withheld.

Subscribe to sync error events so this surfaces immediately. See [Connections needing relink](/how-to/reliability/detect-a-connection-that-needs-relinking).

### If this didn't work

<AccordionGroup>
  <Accordion title="The relink completes but the sync fails the same way">
    The credentials are valid but lack the access the sync needs. That's a permission problem wearing an authentication costume - see [Permission errors](/how-to/troubleshooting/resolve-a-permission-error).
  </Accordion>
...
- [How to connect](/explanation/magic-link) - how a connector is created in the first place
- [Sync Status](/how-to/troubleshooting/monitor-sync-status) - the other half of connector health
- [Errors & Issues](/explanation/errors-issues-and-limits) - where failures surface
