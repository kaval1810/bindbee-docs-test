...
flowchart LR
    ...
    INT --> CON["Connector<br/>one customer's connection"]
...
The same levels tell you where a setting lives, and how far a change to it reaches. Above them sits your **organisation** - your company's account - holding two **environments**, Development and Production, with separate connectors, keys and logs.
...
<Warning>
Environments separate *data*, not *configuration*. Scope and custom fields are organisation-level, so **a change made while testing in Development is a change in Production too.** Decide your scope before you start testing, not after - see [Environments](/get-started/environments).
</Warning>
