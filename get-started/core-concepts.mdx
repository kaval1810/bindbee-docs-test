...
```mermaid
flowchart LR
    CAT["Category<br/>HRIS · ATS · LMS"] --> INT["Integration<br/>BambooHR, Workday"]
    INT --> CON["Connector<br/>A customer's connection"]
    CON --> MOD["Model<br/>employee, benefit"]
    MOD --> REC["Record<br/>one synced instance"]
```

Each level narrows the one before it. A **category** contains the **integrations** Bindbee supports for that kind of system; an integration has one **connector** per customer who uses it; a connector syncs the **models** its category defines; and each model holds the **records** belonging to that customer.
...
| **Connector token** | Names which customer's connection a request acts on. Sent as `X-Connector-Token`. Required wherever the request reaches one customer's data. Distinct from the connector's `id`, which is how a URL names the connector itself |
...
<Note>
A vendor can be offered in more than one category, so a connector is pinned to a category as well as an integration. A customer using SAP SuccessFactors for both HR and recruiting has **two** connectors, not one - see [Integration & Coverage](/get-started/integrations).
</Note>

Anything reaching one customer's data needs both credentials: the key for authority, the token for subject. Which endpoints need which is in [Authentication](/api-reference/basics/authentication).

## Where settings live

The same levels tell you where a setting lives, and how far a change to it reaches. Category and model hold no settings - configuration hangs off the three levels between them, with your **organisation** at the top.

| Set at | What lives there | A change reaches |
| --- | --- | --- |
| **Organisation** | Scope, custom field definitions | Every customer, both environments |
| **Integration** | Custom field mappings | Every customer on that vendor system |
| **Connector** | Credentials, sync schedule, mapping overrides | One customer |

A mapping set on a connector wins over the one set on its integration - see [Two scopes, and which one wins](/guides/extending/custom-fields).

<Warning>
**Environments are not a fourth level.** Development and Production partition connectors, keys, records and logs - not configuration. Scope and custom field definitions are organisation-level, so **a change made while testing in Development is a change in Production too.** Decide your scope before you start testing, not after - see [Environments](/get-started/environments).
</Warning>

## Normalisation is opinionated

Levels and tokens settle *whose* data a request reaches. The rest of this page is about *what shape* it arrives in.
...
<Warning>
Both `raw_data` and `custom_fields` default to **off**. A mapped custom field does not appear in a response until you ask for it, which is the usual reason a mapping looks like it silently failed. See [Raw Data](/guides/reading-writing/raw-data).
</Warning>

Raw data is where you go when a unified field is empty, when the fallback chain picked the wrong source, or when the value has no unified equivalent at all. It is deliberately not normalised, so **reading it directly in application code means writing integration-specific handling** - the thing you adopted a unified API to avoid. Promote what you need into a [custom field](/guides/extending/custom-fields) instead; for when to reach for which, see [Raw data](/guides/reading-writing/raw-data).

## Coverage varies by integration

**Not every integration supports every model.** Coverage follows what the source system exposes and differs integration by integration, even within one category. Check [Model availability](/get-started/model-availability) before you commit to a workflow, then confirm it against a real connector - see [Check a connector's actual coverage](/get-started/integrations).

**Some models are still settling.** Any model carrying a **BETA** badge works, but its field names and shapes are not yet frozen. Pin what you depend on and expect to revisit it.
