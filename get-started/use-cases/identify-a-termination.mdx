...
<Steps>
  ...
  <Step title="Match every terminal status">
    ...
    `INACTIVE_EXTERNAL` is a contractor ending an engagement. Matching only on `INACTIVE` silently misses them - see [Filtering](/guides/reading-writing/querying-data) for the full enum.
