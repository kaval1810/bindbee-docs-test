...
When you call `GET /api/hris/v1/employees`, Bindbee does **not** call BambooHR. It reads from a normalized copy that a background sync job produced earlier - and that's what makes the API reliable:

- Consistent response shape across every connector
- Predictable latency, independent of vendor performance
- Your rate limits, not the vendor's
- Resilience when the vendor has an outage
- One pagination and filtering model

The trade-off is freshness: the API reflects the most recent sync, not the HRIS at this exact second. See [Data Freshness](/concepts/how-syncing-works) for how sync frequency determines how current that is.
...
<Warning>
  ...
  [Connector States & Lifecycle](/concepts/connector-lifecycle).
</Warning>
...
See [Field semantics](/concepts/field-semantics) and [Filtering with `modified_after`](/how-to/reading-data/filter-with-modified-after).
...
5. **Don't resync on a timer.** It does not make data fresher than the vendor allows, and it burns rate limit. See [Force a resync](/how-to/sync-control/force-a-resync).

## Related

<CardGroup cols={2}>
  <Card title="Data Freshness & Polling" icon="clock" href="/concepts/how-syncing-works">
    How current is the data, really?
  </Card>
  <Card title="Connector States & Lifecycle" icon="route" href="/concepts/connector-lifecycle">
